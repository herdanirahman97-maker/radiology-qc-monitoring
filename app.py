...
def build_room_html(sheet_name, file_name, df, logo_base64):
    ...
    <div class="page-container">
        <div style='display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; color: black; border-bottom: 2px solid #002B5B; padding-bottom: 12px;'>
            ...
            <div style='flex: 1 1 auto; text-align: center;'>
                <h2 style='margin: 0; font-weight: 700; font-size: 24px; letter-spacing: 0.5px;'>Form Digital Monitoring Suhu dan Kelembapan</h2>
                <h4 style='margin: 6px 0 0 0; font-weight: 600; font-size: 15px; color: #333;'>Departemen Radiologi Tahun 2026</h4>
            </div>
            <div style='flex: 0 0 190px;'></div>
        </div>
        
        <div style='font-weight: 700; font-size: 14px; margin-bottom: 12px; color: black;'>
            RUANG : {ruang_name}<br>
    ...
    # --- SUHU SECTION ---
    html += f"<tr><th rowspan='3' style='border: 1px solid black; background-color: {header_bg}; color: {header_fg}; font-weight: 700; width: 50px; vertical-align: middle; font-size: 13px;'>SUHU</th>"
    html += f"<th colspan='93' style='border: 1px solid black; background-color: {header_bg}; color: {header_fg}; font-weight: 700; padding: 8px; font-size: 12px;'>Target Temperatur (18 - 23)°C</th></tr>"
    
    html += f"<tr>"
    for day in range(1, 32):
        bg = "#E0F7FA" if day % 2 == 0 else "#FFFFFF"
        html += f"<td colspan='3' style='border: 1px solid black; font-weight: 700; background-color: {bg}; color: black; padding: 6px; font-size: 11px;'>{day}</td>"
    html += "</tr>"
    ...
    for day in range(1, 32):
        bg = "#E0F7FA" if day % 2 == 0 else "#FFFFFF"
        for shift in ['P', 'S', 'M']:
            html += f"<td style='border: 1px solid black; font-weight: 700; background-color: {bg}; color: black; padding: 5px; font-size: 10px;'>{shift}</td>"
    html += "</tr>"
    ...
    for temp in range(30, 14, -1):
        ...
        html += f"<td style='border: 1px solid black; background-color: {label_bg}; font-weight: 700; color: {label_color}; padding: 6px; font-size: 11px;'>{temp}</td>"
        for day in range(1, 32):
            bg = "#E0F7FA" if day % 2 == 0 else "#FFFFFF"
            for shift in ['P', 'S', 'M']:
                ...
                html += f"<td style='border: 1px solid black; background-color: {bg}; padding: 0; height: 32px;'>{dot}</td>"
        html += "</tr>"
        
    # --- KELEMBAPAN SECTION ---
    html += f"<tr><th rowspan='3' style='border: 1px solid black; background-color: {header_bg}; color: {header_fg}; font-weight: 700; vertical-align: middle; font-size: 13px;'>KELEMBAPAN</th>"
    html += f"<th colspan='93' style='border: 1px solid black; background-color: {header_bg}; color: {header_fg}; font-weight: 700; padding: 8px; font-size: 12px;'>Target kelembapan ( 40 - 60 % )</th></tr>"
    ...
    for hum in range(65, 29, -5):
        ...
        html += f"<td style='border: 1px solid black; background-color: {label_bg}; font-weight: 700; color: {label_color}; padding: 6px; font-size: 11px;'>{hum}</td>"
        for day in range(1, 32):
            bg = "#E0F7FA" if day % 2 == 0 else "#FFFFFF"
            for shift in ['P', 'S', 'M']:
    ...
    html += f"<tr><td style='border: 1px solid black; background-color: {nama_bg}; color: black; font-weight: 700; font-size: 11px; padding: 6px;'>NAMA</td>"
    for day in range(1, 32):
        for shift in ['P', 'S', 'M']:
            nama = data_dict.get((day, shift), {}).get('nama', '')
            html += f"<td style='border: 1px solid black; background-color: {nama_bg}; color: #002B5B; font-size: 9px; font-weight: 700; padding: 4px; height: 34px;'>{nama}</td>"
    html += "</tr>"
